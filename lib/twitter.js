var VERSION = '0.2.8',
  ...
  utils = require('./utils');
  keys = require('./keys'),
  RateLimiter = require('./node-rate-limiter').RateLimiter;
...
function Twitter(options) {
  ...
  // add a rate limiter to respect Twitter's API request rate restrictions
  this.limiter = new RateLimiter(this.options.requests_per_hour, 'hour');  
 ...
 * UPDATE RATE LIMITER
 */
Twitter.prototype.updateRateLimits = function(headers, callback) {
  if ((typeof(headers['x-ratelimit-limit']) != 'undefined') && (typeof(headers['x-ratelimit-remaining']) != 'undefined')) {
    var limit = parseInt(headers['x-ratelimit-limit']);
    var remaining = parseInt(headers['x-ratelimit-remaining']);
  
    this.limiter.tokenBucket.bucketSize         = limit;
    this.limiter.tokenBucket.tokensPerInterval  = limit;
    this.limiter.tokenBucket.maxBurst           = limit;

    this.limiter.tokenBucket.content = remaining;
    
    process.send({ cmd: 'updateTwitterRateLimits', data: this.limiter.tokenBucket });
  }
  callback();
}
...
Twitter.prototype.get = function(url, params, callback) {
  ...
  this.limiter.removeTokens(1, function(err, remainingRequests) {
    self.oauth.get(url + '?' + querystring.stringify(params),
      self.options.access_token_key,
      self.options.access_token_secret,
    function(error, data, response) {
      if (typeof(response) != 'undefined') {
        self.updateRateLimits(response.headers, function() {});
      }
 ...
 */
Twitter.prototype.search = function(params, callback) {
  if (typeof params === 'function') {
    callback = params;
    params = {};
  }
